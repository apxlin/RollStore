...
class ClientAgent:
    ...
    def run(self):
        ...
        with grpc.insecure_channel('localhost:50052', options=options) as channel:
            stub = psgrpc.ProverNodeStub(channel)

            Task = ps.Task(current_root = strcroot, current_width = cwidth, current_peaksx = peaksxstr, current_peaksy = peaksystr, items_to_updatex = itemsxstr, 
            items_to_updatey =itemsystr, new_root=strnroot)
            response = stub.Execute(Task)
